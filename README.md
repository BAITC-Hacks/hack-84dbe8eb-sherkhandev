...
### Сценарий для жюри в браузере

Предусловия: сервер запущен в режиме `fixture`, настроен рабочий ключ и доступная модель OpenAI; открыта новая вкладка с интерфейсом.

1. Выберите **«Учебный склад»** (`demo-warehouse`).
2. Отправьте: **«Найди DEMO-001, покажи его характеристики и остаток на выбранном складе»**. В данных товара: «Учебный автомат A», цена `1200.00 KZT`, остаток `5` шт., источник `synthetic`.
3. Отправьте: **«Добавь 2 штуки DEMO-001 в корзину»**. Должно появиться предложение: 2 шт. × 1200.00 KZT = **2400.00 KZT**. До подтверждения корзина остаётся пустой.
4. Нажмите **«Подтвердить добавление»** до истечения предложения — по умолчанию в течение 5 минут. В корзине должна появиться одна строка с количеством 2 и общей суммой **2400.00 KZT**.
5. Откройте ссылку просмотра под корзиной. Она показывает состав и сумму без элементов изменения заказа; срок действия ссылки по умолчанию — 30 минут с момента её выдачи.

Формулировки ответов модели могут различаться; проверять следует данные товара, предложение и итог корзины.

Дополнительные запросы:

| Запрос | Что проверить |
| --- | --- |
| «Покажи сертификат DEMO-001» | В данных есть ссылка на `DEMO-CERT-001.html`; документ явно обозначен учебным |
| «Есть ли DEMO-002? Подбери аналог на выбранном складе» | У DEMO-002 остаток 0; DEMO-003 подходит по учебным правилам, имеет остаток 10 и цену 1500.00 KZT |
| «Какие условия оплаты, доставки и минимальной партии?» | Ответ опирается на демонстрационные условия: оплата по счёту, самовывоз, минимальная партия 1 шт. |

Учебный сертификат также можно открыть напрямую: [DEMO-CERT-001](http://127.0.0.1:8000/demo-certificates/DEMO-CERT-001.html).
...
В тестах используются подставные ответы LLM и HTTP-ответы EKT. Они проверяют локальную логику и взаимодействие компонентов, но не доказывают доступность реальных внешних сервисов.
...
Параметр `--product-id` можно повторять. Импорт и сервер должны использовать одинаковый `EKT_DATA_DIR`. Доступность endpoint и совместимость фактического ответа EKT с адаптером не подтверждены успешной проверкой реального сервиса в репозитории. При ошибке свежего запроса сервис возвращает `SOURCE_UNAVAILABLE`, не выдавая старый снимок за свежий.
...
- Каталог — ограниченная локальная выборка. Полного импорта и автоматической синхронизации каталога нет; доступность настоящей интеграции EKT требует отдельной проверки.
...
Публичная ссылка на развёрнутую версию в текущем репозитории не указана. Подтверждённый кодом способ запуска — локальный сервер по инструкции выше: [http://127.0.0.1:8000/](http://127.0.0.1:8000/).
